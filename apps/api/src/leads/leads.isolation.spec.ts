...
const META: GatewayRequestMeta = { ip: "127.0.0.1", requestId: "req-leads-isolation" };
...
describe("LeadsService tenant isolation", () => {
  it("tenant A reads only its own leads and never hits tenant B's site", async () => {
    ...
    const detail = await service.detail(USER_A, META, "LD-A-001");
    expect(detail.code).toBe("LD-A-001");
    expect(detail.company).toBe("Acme Corp");

    const list = await service.list(USER_A, META, { page: 1, pageSize: 20 });
    expect(list.items.some((lead) => lead.code === "LD-B-001")).toBe(false);

    expect(siteA.requests.length).toBeGreaterThan(aRequestsBefore);
    expect(siteB.requests).toHaveLength(bRequestsBefore);
  });

  it("returns not_found (404) for tenant B's lead and never calls tenant B's site", async () => {
    mockTenant(COMPANY_A, { host: siteA.url, serviceKeyCipher: cipher(KEY_A.apiKey, KEY_A.apiSecret) });
    const bRequestsBefore = siteB.requests.length;

    await expectApiException(service.detail(USER_A, META, "LD-B-001"), ErrorCode.NOT_FOUND, 404);

    expect(siteB.requests).toHaveLength(bRequestsBefore);
  });

  it("creates a lead on tenant A's site only, using tenant A's service account", async () => {
    mockTenant(COMPANY_A, { host: siteA.url, serviceKeyCipher: cipher(KEY_A.apiKey, KEY_A.apiSecret) });
    ...
    expect([...siteA.docs.values()].some((doc) => doc.company_name === "A-Only Ltd")).toBe(true);
  ...
  it("moves a lead's stage on tenant A's site only", async () => {
    mockTenant(COMPANY_A, { host: siteA.url, serviceKeyCipher: cipher(KEY_A.apiKey, KEY_A.apiSecret) });
    const bDocsBefore = [...siteB.docs.keys()];
    const bRequestsBefore = siteB.requests.length;

    await service.moveStage(USER_A, META, "LD-A-001", { stage: "qualified" });

    expect(siteA.docs.get("LD-A-001")?.status).toBe("Qualified");
    expect([...siteB.docs.keys()]).toEqual(bDocsBefore);
    expect(siteB.requests).toHaveLength(bRequestsBefore);
  });
});
